...
const SubscribedLists = (props) => {
  const [list, setList] = useState({});
  const [isEditing, setEditing] = useState(false);

  // handles the removal of a list from the backend
  const handleUnsubscribe = () => {
    console.log("Unsubscribe from this list", list);
          ...
          <Disclosure.Panel className="px-4 py-2">
            ...
            >
              ...
              <div className="mt-4 text-gray-900">
                ...
                <div className="w-full border rounded-md px-2 py-1 outline-none focus:ring-base-blue focus:ring-2 focus:ring-opacity-80">
                  {list.description}
                </div>
                <div className="grid grid-cols-2 gap-2 mt-2">
                  <div>
                    <h3 className="tracking-wider">Owner</h3>
                    <div className="cols-span-1 w-full border rounded-md px-2 py-1 outline-none focus:ring-base-blue focus:ring-2 focus:ring-opacity-80">
                      {list.owner}
                    </div>
                  </div>
                  <div>
